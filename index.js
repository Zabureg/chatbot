...
client.on('message', message => {
    ...
    if(commandName == "info") {
        ...
        if(args[0] == "17"){
            ...
            text = `${prefix}weather город - показать погоду города на данный момент.`;
            console.log(`${message.author.username}(${message.author.id}) send command info for bot!`);
            message.channel.send(infomessage(color, title, text));
            return;
        }
        if(args[0] == "18"){
            ...
            text = `${prefix}clear число - удаляет сообщения бота и сообщения с командами, остальные не трогает..`;//
            console.log(`${message.author.username}(${message.author.id}) send command info for bot!`);
            message.channel.send(infomessage(color, title, text));
            return;
        }
